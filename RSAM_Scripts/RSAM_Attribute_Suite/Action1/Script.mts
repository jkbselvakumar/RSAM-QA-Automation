' ‘################################################################################################################
' #Script Name:	RSAM _Attribute_Pagination_Search
...
'# ------		--------					---------------
'################################################################################################################## 
	Dim pageFlag, counter, clrAttTyp, oAttFound
	pageFlag =1
	counter =1
	Do While Browser("RSAM").Page("RSAM Administration").Frame("AttributeFrame").Link("Next").Exist or counter = 1
		...
		For i=2 to rows
			...
			If strAttTypeName = Parameter("attType_Name") Then
				...
				If clrAttTyp = "yellow"  Then
					...
					Browser("RSAM").Page("RSAM Administration").Frame("AttributeFrame").WebTable("AttributeTypeQuestions").WebElement("innerhtml:="&strAttTypeName&"","innertext:="&strAttTypeName&"").Click
				Else
		...
		End If
	Loop
	If oAttFound = False Then
		RSAM_Automation_Result_File_Update "Pagination_Search","Validation of the created attribute type",""&strAttTypeName&" should be found","Not Found"&strAttTypeName&"","Fail"
	End If
'################################################################################################################## 
